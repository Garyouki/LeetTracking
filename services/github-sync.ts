...
const GIST_FILENAME = 'leettracking-backup.json';
const LEGACY_GIST_FILENAME = 'leetsrs-backup.json';
...
export async function validateGistId(gistId: string, pat: string): Promise<GistValidationResult> {
  ...
  try {
    ...
    if (!data.files?.[GIST_FILENAME] && !data.files?.[LEGACY_GIST_FILENAME]) {
      return { valid: false, error: `Gist does not contain ${GIST_FILENAME} or ${LEGACY_GIST_FILENAME}` };
    }
...
export async function triggerGistSync(): Promise<SyncResult> {
  ...
  try {
    ...
    const remoteFileContent =
      remoteGist.files?.[GIST_FILENAME]?.content ?? remoteGist.files?.[LEGACY_GIST_FILENAME]?.content;
