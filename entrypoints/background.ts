...
import { syncReview } from '@/services/notion-sync';
  ...
  async function handleMessage(request: MessageRequest) {
    ...
    switch (request.type) {
      ...
      case MessageType.SYNC_TO_NOTION:
        console.log('Received SYNC_TO_NOTION message:', request);
        try {
          ...
          console.log('Notion sync completed successfully, result:', result);
          return result;
        } catch (e) {
